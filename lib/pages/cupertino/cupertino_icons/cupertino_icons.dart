...
class CupertinoIconsScreen extends StatefulWidget {
  const CupertinoIconsScreen({super.key});

  static const routeName = 'cupertino/CupertinoIcons';
  static const title = 'CupertinoIcons';

  @override
  State<CupertinoIconsScreen> createState() => _CupertinoIconsScreenState();
}

class _CupertinoIconsScreenState extends State<CupertinoIconsScreen> {
  var _keyword = '';
  var _keywordInputController = TextEditingController(text: '');

  Map<String, IconData> _icons = _Data.icons;
  bool isList = true;

  void _keywordOnChange(String keyword) {
    _keyword = keyword;
    ...
    if (keyword == '') {
      ...
      icons = <String, IconData>{
        for (var key in _Data.icons.keys)
          if (key.startsWith(keyword)) key: _Data.icons[key]!,
        for (var key in _Data.icons.keys)
          if (key.contains(keyword) && !key.startsWith(keyword))
            key: _Data.icons[key]!
      };
    }
  ...
  void toggleIsList() {
    setState(() {
      isList = !isList;
      _keywordInputController = TextEditingController(text: _keyword);
    });
  }
  ...
  Widget build(BuildContext context) {
    return CupertinoPageScaffold(
      navigationBar: CupertinoNavigationBar(
        middle: const Text(CupertinoIconsScreen.title),
        border: null,
      ...
      child: isList
          ? ListView(
              children: [
                _KeywordInput(
                  onChanged: _keywordOnChange,
                  controller: _keywordInputController,
                ),
                for (var key in _icons.keys) _ListTile(iconName: key),
              ],
            )
          : GridView.count(
              crossAxisCount: 5,
              children: [
                // TODO: 添加 _KeywordInput
                for (var key in _icons.keys) _GridItem(iconName: key),
              ],
            ),
    );
  }
...
class _ListTile extends StatelessWidget {
  ...
  const _ListTile({required this.iconName});
    ...
    return CupertinoListSection(
      ...
      children: [
        CupertinoListTile.notched(
          title: Text(iconName),
          leading: Icon(_Data.icons[iconName], size: 32),
          onTap: () => Clipboard.setData(
              ClipboardData(text: 'CupertinoIcons.$iconName')),
        ),
      ],
    );
  }
...
class _GridItem extends StatelessWidget {
  final String iconName;
  const _GridItem({required this.iconName});
  ...
  Widget build(BuildContext context) {
    return CupertinoButton(
      padding: EdgeInsets.zero,
      minSize: 0,
      child: Icon(_Data.icons[iconName], size: 32),
      onPressed: () =>
          Clipboard.setData(ClipboardData(text: 'CupertinoIcons.$iconName')),
    );
  }
}
